...
public actor CrashTracePreserver {
    /// 링 버퍼 (최근 N개 로그 보관)
    private var ringBuffer: RingBuffer<TraceMessage>
    ...
    /// 저장된 로그 복구 (앱 시작 시 호출)
    public func recover() throws -> [TraceMessage]? {
        // 먼저 mmap 크래시 데이터 확인
        if hasCrashData() {
        ...
        let messages = try decoder.decode([TraceMessage].self, from: data)
    ...
    /// 현재 버퍼 내용 확인
    public func currentLogs() -> [TraceMessage] {
        ringBuffer.toArray()
    }
...
public extension CrashTracePreserver {
    /// 크래시 시그널 핸들러 등록
    /// - Note: SIGABRT, SIGSEGV 등 처리
    /// - Parameter preserver: 크래시 로그를 저장할 preserver
    static func registerSignalHandlers(preserver _: CrashTracePreserver) {
