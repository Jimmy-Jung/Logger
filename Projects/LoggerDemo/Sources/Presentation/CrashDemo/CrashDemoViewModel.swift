...
final class CrashDemoViewModel: ObservableObject {
    ...
    @Published var recoveredLogs: [LogMessage] = []
    @Published var crashMarkerDetected: Bool = false
    @Published var statusMessage: String = ""

    // MARK: - Private Properties

    private let preserver: CrashLogPreserver
    private var logTimer: Timer?

    // MARK: - Initialization

    init() {
        preserver = CrashLogPreserver(preserveCount: 50)
    ...
    func checkPreviousCrash() async {
        do {
            ...
            if let logs = try await preserver.recover() {
                ...
                // 로그 내용 출력
                await Logger.async.warning("이전 크래시 복구: \(logs.count)개 로그", category: "Crash")
            } else {
            ...
            await Logger.async.error("크래시 로그 복구 실패: \(error)", category: "Crash")
        }
    }
    ...
    func startRecording() async {
        ...
        logTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
            ...
            Task { @MainActor in
                ...
                let level: LogLevel = [.debug, .info, .warning].randomElement() ?? .info
                ...
                await Logger.async.log(level: level, message, category: "CrashDemo")
            }
        }
    }
    ...
    func saveToFile() async {
        do {
            ...
            await Logger.async.info("로그 파일 저장 완료", category: "Crash")
        } catch {
            statusMessage = "⚠️ 저장 실패: \(error.localizedDescription)"
            await Logger.async.error("로그 저장 실패: \(error)", category: "Crash")
        }
    }
    ...
    func simulateCrash() async {
        ...
        await Logger.async.fatal("크래시 시뮬레이션 실행", category: "Crash")
    ...
    func triggerRealCrash() async {
        ...
        await Logger.async.fatal("실제 크래시 발생!", category: "Crash")

        // 약간의 지연 (로그 저장 시간 확보)
        try? await Task.sleep(nanoseconds: 200_000_000)

        // 실제 크래시 발생!
        fatalError("💥 Intentional crash for testing CrashLogPreserver")
    }
    ...
    func triggerNSExceptionCrash() async {
        ...
        await Logger.async.fatal("NSException 크래시 발생!", category: "Crash")
        try? await Task.sleep(nanoseconds: 200_000_000)
    ...
    func triggerForceUnwrapCrash() async {
        ...
        await Logger.async.fatal("Force Unwrap 크래시 발생!", category: "Crash")
        try? await Task.sleep(nanoseconds: 200_000_000)
    ...
    func triggerArrayCrash() async {
        ...
        await Logger.async.fatal("Array Index 크래시 발생!", category: "Crash")
        try? await Task.sleep(nanoseconds: 200_000_000)
    ...
    func clearLogs() async {
        do {
            ...
            await Logger.async.info("로그 클리어됨", category: "Crash")
        } catch {
    ...
    /// 현재 로그 목록 가져오기
    func getCurrentLogs() async -> [LogMessage] {
